...
setwd("D:/T1DGC/USERS/dam8mt/data/T1DGC_Humancore_Exome/HLA_imputation/haplotype_analysis/AMR/output")
library(rtf)
library(lmtest)
...
pcs=read.table("D:/T1DGC/USERS/dam8mt/data/T1DGC_Humancore_Exome/HLA_imputation/haplotype_analysis/AMR/input/T1DGC_HCE_pc_AMR.txt", header=TRUE, comment.char="~"); rownames(pcs) <- pcs$IID
...
geno_2 = read.table("D:/T1DGC/USERS/dam8mt/data/T1DGC_Humancore_Exome/HLA_imputation/haplotype_analysis/AMR/input/T1DGC_HCE_imp_AMR_chr6.fam", comment.char="~", header=F)
...
erlichfile = "D:/T1DGC/USERS/dam8mt/data/T1DGC_Humancore_Exome/HLA_imputation/haplotype_analysis/AMR/input/diab.2008_Erlich.table2_v2.txt"
...
top_hap = NA
while(p_min<p_thresh) { # instead of p_thresh type 0.1 to include the second independent loci
...
addParagraph(rtffile, "Supplementary Table ##. Association of T1D in Admixed population with MHC Class II Haplotypes")
...
addParagraph(rtffile, "Supplementary Table 7. xx HLA class II haplotypes are independently associated with T1D in Admixed population")
